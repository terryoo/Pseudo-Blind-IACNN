...
Est_Par.mbSize = 64;
Est_Par.stride = 64;
Est_Par.net_model = 'deploy/Est_avc_deploy.prototxt';
Est_Par.net_weights = 'model_avc/avc_est.caffemodel';
...
%% Load CNN Models
Par.net_model = 'deploy/inception_deploy.prototxt';
Net_est = caffe.Net(Est_Par.net_model, Est_Par.net_weights, 'test');
Net34 = caffe.Net(Par.net_model, 'model_avc/34.caffemodel', 'test');
Net37 = caffe.Net(Par.net_model, 'model_avc/37.caffemodel', 'test');
Net42 = caffe.Net(Par.net_model, 'model_avc/42.caffemodel', 'test');
Net47 = caffe.Net(Par.net_model, 'model_avc/47.caffemodel', 'test');
Net51 = caffe.Net(Par.net_model, 'model_avc/51.caffemodel', 'test');

%% Test Sets & Save Folder 
testfolder = './testset/avc/comp47/BQTerrace/';
GTfoler = './testset/original/BQTerrace/';
save_folder = './results/';
imglist = dir(fullfile(testfolder,'*.png'));
imglist_gt = dir(fullfile(GTfoler,'*.png'));
...
for i=1:length(imglist)    
    ...
    elseif(est_QP == 47)
        ...
        imgout = IACNN(currframe_Y,Net51);     
        estmat(i) = 51;
    end 
    
    gt_frame = imread([GTfoler,imglist_gt(i).name]);  
